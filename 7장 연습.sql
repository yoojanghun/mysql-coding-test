...
-- CASE문을 사용할 때, 아래처럼 어떤 문자열이나 숫자를 대입할 수도 있지만, 컬럼의 값(HEIGHT 값, BIRTH_DATE 값 등)을 대입할 수도 있다.
SELECT 	PLAYER_NAME,
		CASE POSITION
			WHEN 'FW' THEN HEIGHT
			WHEN 'DF' THEN BIRTH_DATE
...
-- 팀 별로 각각 태어난 달에 대한 선수의 평균 키를 구함 (세로축: 팀, 가로축: 태어난 달)
SELECT	TEAM_ID,
		COUNT(*) '선수 수',
		ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 1 THEN HEIGHT END), 2) AS '1월',
		ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 2 THEN HEIGHT END), 2) AS '2월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 3 THEN HEIGHT END), 2) AS '3월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 4 THEN HEIGHT END), 2) AS '4월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 5 THEN HEIGHT END), 2) AS '5월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 6 THEN HEIGHT END), 2) AS '6월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 7 THEN HEIGHT END), 2) AS '7월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 8 THEN HEIGHT END), 2) AS '8월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 9 THEN HEIGHT END), 2) AS '9월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 10 THEN HEIGHT END), 2) AS '10월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 11 THEN HEIGHT END), 2) AS '11월',
        ROUND(AVG(CASE WHEN MONTH(BIRTH_DATE) = 12 THEN HEIGHT END), 2) AS '12월',
        ROUND(AVG(CASE WHEN BIRTH_DATE IS NULL THEN HEIGHT END), 2) AS UNDECIDED
FROM	PLAYER
GROUP	BY TEAM_ID
ORDER	BY TEAM_ID;


-- 참고로 SUM(HEIGHT)에서 HEIGHT가 모두 NULL이면, SUM(HEIGHT)는 NULL을 리턴함.